...
export async function listAdminTickets(options: ListAdminTicketsOptions = {}): Promise<AdminTicket[]> {
  const qs: string[] = ['includeAll=1'];
  if (typeof options.limit === 'number') qs.push(`limit=${encodeURIComponent(String(options.limit))}`);
  if (typeof options.offset === 'number') qs.push(`offset=${encodeURIComponent(String(options.offset))}`);
  if (options.sellerId === null) qs.push('sellerId=__unassigned__');
  if (typeof options.sellerId === 'number' && Number.isFinite(options.sellerId)) {
    qs.push(`sellerId=${encodeURIComponent(String(options.sellerId))}`);
  }
  if (options.status) qs.push(`status=${encodeURIComponent(options.status)}`);
  if (options.startDate) {
    qs.push(`startDate=${encodeURIComponent(options.startDate)}`);
    qs.push(`fromDate=${encodeURIComponent(options.startDate)}`);
  }
  if (options.endDate) {
    qs.push(`endDate=${encodeURIComponent(options.endDate)}`);
    qs.push(`toDate=${encodeURIComponent(options.endDate)}`);
  }
  const response = await requestJson<unknown>(`/admin/tickets?${qs.join('&')}`, { method: 'GET' });
