...
function createAuthRouter({ db, hashPassword, verifyPassword, getQrState }) {
  ...
  function readAdminCount() {
    try {
      return Number(db.prepare("SELECT COUNT(*) as count FROM users WHERE role = 'admin'").get().count || 0);
    } catch (_) {
      return 0;
    }
  }
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      try {
        const adminCount = readAdminCount();
        if (adminCount === 0) {
          return res.status(409).json({
            error: 'Nenhum administrador cadastrado. Crie o primeiro admin para iniciar.',
            setupRequired: true,
          });
        }
  ...
  router.get('/auth/has-admin', (_req, res) => {
    try {
      const count = readAdminCount();
      return res.json({ hasAdmin: count > 0, adminCount: count });
    } catch (_error) {
