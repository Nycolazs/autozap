...
const ROOT_DIR = storagePaths.resolveProjectRootDir();
const DATA_DIR = storagePaths.resolveDataDir();
...
const LEGACY_DB_DIR = path.join(LEGACY_DATA_DIR, 'db');
...
const LEGACY_ACTIVE_ACCOUNT_FILE = path.join(LEGACY_DATA_DIR, 'active-account.json');
const LEGACY_ACCOUNTS_DIR = path.join(LEGACY_DATA_DIR, 'accounts');
const LEGACY_STAGING_DIR = path.join(LEGACY_DATA_DIR, 'staging');

function bootstrapDataDirFromLegacyIfNeeded() {
  try {
    ...
    if (!fs.existsSync(ACTIVE_ACCOUNT_FILE) && fs.existsSync(LEGACY_ACTIVE_ACCOUNT_FILE)) {
      copyFileIfExists(LEGACY_ACTIVE_ACCOUNT_FILE, ACTIVE_ACCOUNT_FILE);
    }

    if (!fs.existsSync(ACCOUNTS_DIR) && fs.existsSync(LEGACY_ACCOUNTS_DIR)) {
      tryRenameOrCopyDir(LEGACY_ACCOUNTS_DIR, ACCOUNTS_DIR);
    }

    if (!fs.existsSync(STAGING_DIR) && fs.existsSync(LEGACY_STAGING_DIR)) {
      tryRenameOrCopyDir(LEGACY_STAGING_DIR, STAGING_DIR);
    }
  } catch (_) {}
}

bootstrapDataDirFromLegacyIfNeeded();
...
function extractNumberFromSessionUser(sockUser) {
...
function extractNumberFromCredsDir(authDir) {
  // Estado de autenticação legado salva creds.json com creds.me.id
  try {
