...
const firebaseMirror = require('./firebase/sqliteMirrorSync');
...
function getActiveAccountId() {
  return (accountContext.getActiveAccount && accountContext.getActiveAccount()) || 'default';
}

function isMutatingSql(sql) {
  const normalized = String(sql || '').trim().toUpperCase();
  return /^(INSERT|UPDATE|DELETE|REPLACE|ALTER|CREATE|DROP|TRUNCATE|VACUUM)\b/.test(normalized);
}

function wrapPreparedStatement(stmt, trackMutations) {
  if (!trackMutations) return stmt;
...
function ensureCurrentDb() {
  ...
  firebaseMirror.setContextResolver(() => ({
    db: currentDb,
    accountId: getActiveAccountId(),
  }));
  firebaseMirror.bootstrapAccount(currentDb, getActiveAccountId());
  return currentDb;
}
...
module.exports = new Proxy({}, {
  get(_target, prop) {
    ...
    if (prop === 'close') return () => {
      try { firebaseMirror.flushNow(); } catch (_) {}
      if (currentDb) {
        try { currentDb.close(); } catch (_) {}
      }
      currentDb = null;
    ...
    if (prop === 'prepare') {
      return (sql) => {
        ...
        return wrapPreparedStatement(stmt, isMutatingSql(sql));
      };
    }
    ...
    if (typeof v === 'function') {
      const bound = v.bind(db);
      if (MUTATION_HELPERS.has(prop)) {
        return (...args) => {
