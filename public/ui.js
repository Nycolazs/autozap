...
(function () {
  ...
  window.ensureConnected = async function ensureConnected() {
    async function fetchAdminState() {
      return window.smartJson('/auth/has-admin', { cache: 'no-store', maxRetries: 1 });
    }
    ...
    // Onboarding inicial: sem admin, envia para tela de boas-vindas.
    try {
      const adminData = await fetchAdminState();
      if (!adminData.hasAdmin) {
        if (typeof navigateTo === 'function') navigateTo('/welcome'); else window.location.href = '/welcome';
        return false;
      }
    } catch (_) {
      showNotification('Não foi possível validar o cadastro inicial. Tente novamente.', 'warning', 3200);
      return false;
    }

    // Avoid redirect flapping: allow a short grace period for reconnection.
    // Redirect when backend indicates QR flow or missing Cloud API configuration.
    ...
    while ((Date.now() - startedAt) < maxWaitMs) {
      try {
        ...
        const setupRequired = !!qrData.setupRequired
          || qrData.connectionState === 'missing_config'
          || qrData.connectionState === 'missing_verify_token';

        if (isConnected || isStable) {
          return true;
        }

        if (needsQr || setupRequired) {
    ...
    const setupRequired = !!lastState?.setupRequired
      || lastState?.connectionState === 'missing_config'
      || lastState?.connectionState === 'missing_verify_token';
    if (needsQr || setupRequired) {
      if (typeof navigateTo === 'function') navigateTo('/whatsapp-qr'); else window.location.href = '/whatsapp-qr';
      return false;
    }
